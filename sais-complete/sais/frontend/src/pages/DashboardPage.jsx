...
function DeadlineItem({ a }) {
  ...
  const daysLeft =
    dateObj && !isNaN(dateObj.getTime())
      ? differenceInDays(dateObj, new Date())
      : null;
  const urgent = daysLeft !== null && daysLeft <= 2;
  const dateLabel =
    dateObj && !isNaN(dateObj.getTime()) ? format(dateObj, "MMM d") : null;
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-800 last:border-0">
      <div
        className={`w-2 h-2 rounded-full flex-shrink-0 ${urgent ? "bg-red-400" : "bg-amber-400"}`}
      />
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500">
          ...
          {dateLabel && (
            <span className="ml-1 text-slate-400">· Due {dateLabel}</span>
          )}
        </p>
      </div>
      <span
        className={`text-xs font-mono font-medium flex-shrink-0 ${urgent ? "text-red-400" : "text-amber-400"}`}
      >
        {daysLeft === null
          ? "No date"
          : daysLeft < 0
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    getAlerts(true)
      ...
      .catch(() => {});
    getMorningCheckin()
      .then((r) => setMorningCheckin(r.data))
      .catch(() => {});
    getUnmarkedReminders()
      .then((r) => setUnmarkedReminder(r.data))
      .catch(() => {});
    getEndOfDaySummary()
      .then((r) => setEndOfDaySummary(r.data))
      .catch(() => {});
    client
      .get("/events")
      .then((res) => setAcademicEvents(Array.isArray(res.data) ? res.data : []))
  ...
  async function handleRefreshAlerts() {
    ...
    try {
      ...
      toast.success(
        `Generated ${data.generated} new alert${data.generated !== 1 ? "s" : ""}`,
      );
    } catch {
  ...
  const avgAttendance = summaries.length
    ? Math.round(
        summaries.reduce((s, x) => s + x.attendance_percentage, 0) /
          summaries.length,
      )
    : 0;
  ...
  function normalizeEventType(event) {
    ...
    if (
      ...
      combined.includes("ese") ||
      combined.includes("mse")
    )
      return "exam";
    if (combined.includes("holiday") || combined.includes("break"))
      return "holiday";
    if (combined.includes("semester") || combined.includes("sem"))
      return "semester";
    return "academic";
  }
  ...
  function getEventPillClass(type) {
    ...
    if (type === "exam")
      return "bg-red-400/15 text-red-300 border border-red-400/20";
    if (type === "holiday")
      return "bg-emerald-400/15 text-emerald-300 border border-emerald-400/20";
    if (type === "semester")
      return "bg-blue-400/15 text-blue-300 border border-blue-400/20";
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          sub={
            lowAttendance > 0
              ? `${lowAttendance} subject${lowAttendance > 1 ? "s" : ""} at risk`
              : "All good"
          }
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          {assignmentsForPanel.length === 0 ? (
            ...
            <div>
              {assignmentsForPanel.slice(0, 8).map((a) => (
                <DeadlineItem key={a.id} a={a} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          {alerts.length === 0 ? (
            ...
            <div className="space-y-2">
              {alerts.map((a) => (
                <AlertItem key={a.id} alert={a} onRead={handleReadAlert} />
              ))}
            </div>
          )}
        </div>
              ...
              {summaries.map((s) => (
                <div key={String(s.subject_id)}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span
                      className={`font-mono font-medium ${s.below_threshold ? "text-red-400" : "text-emerald-400"}`}
                    >
                  ...
                  <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${s.below_threshold ? "bg-red-400" : "bg-emerald-400"}`}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span
                      className={`text-xs ${isInMonth ? "text-slate-300" : "text-slate-600"}`}
                    >
                      {format(day, "d")}
                    </span>
                    {isToday(day) && (
                  ...
                  <div className="space-y-1">
                    {dayEvents.slice(0, 2).map((event, idx) => (
                      <div
                        key={`${key}-${idx}`}
                        className={`text-[10px] leading-tight px-1.5 py-1 rounded ${getEventPillClass(event.type)}`}
                      >
                        {event.title}
                      </div>
                    ))}
            ...
            <div className="space-y-2">
              {activities.slice(0, 5).map((a) => (
                <div key={a.id} className="flex items-center gap-3 py-2">
                  <div
                    className={`w-2 h-2 rounded-full flex-shrink-0 ${a.has_conflict ? "bg-red-400" : "bg-blue-400"}`}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-200 truncate">{a.title}</p>
                    <p className="text-xs text-slate-500">
                      {a.activity_date
                        ? format(new Date(a.activity_date), "MMM d")
                        : "TBD"}
