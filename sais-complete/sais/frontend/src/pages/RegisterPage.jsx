...
export default function RegisterPage() {
  ...
  const { strengthText, strengthColor } = useMemo(() => {
    switch (strength) {
      case 0:
        return { strengthText: "", strengthColor: "bg-slate-700" };
      case 1:
        return { strengthText: "Weak", strengthColor: "bg-red-400" };
      case 2:
        return { strengthText: "Moderate", strengthColor: "bg-yellow-400" };
      case 3:
        return { strengthText: "Strong", strengthColor: "bg-green-400" };
      default:
        return { strengthText: "", strengthColor: "bg-slate-700" };
    }
  }, [strength]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      try {
        ...
        const { data } = localAuth.login({ email: normalizedEmail, password });
        await setToken(data.access_token);
      }
            ...
            {password && (
              <div className="mt-2">
                <div className="flex gap-1">
                  <div
                    className={`h-1 flex-1 rounded ${strength >= 1 ? strengthColor : "bg-slate-700"}`}
                  />
                  <div
                    className={`h-1 flex-1 rounded ${strength >= 2 ? strengthColor : "bg-slate-700"}`}
                  />
                  <div
                    className={`h-1 flex-1 rounded ${strength >= 3 ? strengthColor : "bg-slate-700"}`}
                  />
                </div>
