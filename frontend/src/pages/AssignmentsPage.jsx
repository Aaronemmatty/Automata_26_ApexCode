...
export default function AssignmentsPage() {
  ...
  async function load() {
    ...
    if (assignmentsResult.status === 'fulfilled') {
      loadedAssignments = assignmentsResult.value.data || []
    ...
    // Auto-backfill time estimates for assignments that are missing them
    const hasMissing = loadedAssignments.some(a => !a.ai_metadata?.time_estimate)
    if (hasMissing) {
      try {
        const res = await estimateAllAssignments()
        if (res.data?.updated > 0) {
          const refreshed = await getAssignments(filter)
          setAssignments(refreshed.data || [])
          toast.success(`Time estimates generated for ${res.data.updated} assignment(s)`)
        }
          ...
          {documents.length > 0 && (
            <div className="mt-6 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between px-6 py-4 hover:bg-slate-800/30 transition-all"
              >
                <div className="flex items-center gap-3">
                  <FileText size={16} className="text-purple-400" />
                  <span className="text-sm font-semibold text-white">Scraped Documents</span>
                  <span className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded-md">{documents.length}</span>
                        ...
                        <tr key={doc.id} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-all">
                          ...
                          <td className="px-4 py-3">
                            <button
                              ...
                              className="flex items-center gap-1 text-xs text-slate-500 hover:text-amber-400 disabled:opacity-40 transition-all"
                            >
