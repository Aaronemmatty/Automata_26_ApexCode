...
class OllamaTimetableExtractor:
    ...
    async def extract_from_file(self, file_path: str) -> Dict:
        try:
            payload = await self._extract_async(file_path)
            if "error" in payload:
            ...
            logger.exception("Ollama timetable extraction failed, falling back to OCR")
            try:
                ...
                if processed_entries:
                    return {
                        ...
                        "notes": fallback_payload.get("notes", "Used OCR fallback"),
                    }
                ...
                if "tesseract" in fallback_msg and "not found" in fallback_msg:
                    return {
                        "status": "failed",
                        "error": "Ollama extraction failed and OCR fallback is unavailable because Tesseract is not installed. Install Tesseract OCR.",
                        "confidence": 0.0,
                    }
                return {
                    "status": "failed",
                    "error": f"Ollama extraction failed and OCR fallback failed: {fallback_exc}",
                    "confidence": 0.0,
                }
            return {"status": "failed", "error": str(exc), "confidence": 0.0}

    async def _extract_async(self, file_path: str) -> Dict:
        """Extract timetable using Ollama vision API."""
        ...
        # Use Ollama's vision API with llava model
        try:
    ...
    def _extract_with_ocr_fallback_sync(self, file_path: str) -> Dict:
        ...
        return {
            ...
            "notes": "Ollama failed; OCR fallback used",
        }
